...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "drift_summary_custom.csv"
SUMMARY_LONG_FILE: Final[Path] = ARTIFACTS_DIR / "drift_summary_long_custom.csv"
...
REQUESTS_DRIFT_THRESHOLD: Final[float] = 15.0
ERRORS_DRIFT_THRESHOLD: Final[float] = 1.5
LATENCY_DRIFT_THRESHOLD: Final[float] = 800.0
...
def main() -> None:
    ...
    overall_is_drifting_flag_recipe: pl.Expr = (
        pl.col("requests_is_drifting_flag")
        | pl.col("errors_is_drifting_flag")
        | pl.col("latency_is_drifting_flag")
    ).alias("overall_is_drifting_flag")

    drift_df = drift_df.with_columns(
        [
            overall_is_drifting_flag_recipe,
        ]
    )
